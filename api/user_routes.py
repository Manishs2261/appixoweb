...
def get_all_users(db: Session = Depends(get_db)):
    return user_curd.get_all_users_operation(db)


@router.get("/{email}", response_model=users_schemas.UserResponseSchema)
def get_user_by_email(email: str, db: Session = Depends(get_db)):
    db_user = user_curd.get_user_by_email_operation(db, email)
    if not db_user:
...
def create_user_route(new_user: users_schemas.UserCreateSchema, db: Session = Depends(get_db)):
    ...
    if existing_user:
        ...
        return user_curd.create_user_operation(db, new_user)
...
def update_user_route(email: str, user_update: users_schemas.UserUpdateSchema, db: Session = Depends(get_db)):
    update_user = user_curd.update_user_operation(db, email, user_update)
    if not update_user:
        raise HTTPException(status_code=404, detail="User not found")
    return update_user
